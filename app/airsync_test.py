...
print(f"Scraping log detail for {LOG_ID}...")
with sync_playwright() as p:
    browser   = p.chromium.launch(headless=True)
    page      = browser.new_page()
    login(page, email, password)

    # Pull flight list to get origin/destination for this log ID
    from_date = (date.today() - timedelta(days=180)).strftime("%Y-%m-%d")
    flights   = scrape_flights(page, from_date)
    list_row  = next((f for f in flights if f["id"] == LOG_ID), {})

    detail         = scrape_log_detail(page, LOG_ID)
    detail["id"]   = LOG_ID
    detail["date"] = list_row.get("date", "")
    detail["from"] = list_row.get("from", "")
    detail["to"]   = list_row.get("to", "")
    browser.close()
